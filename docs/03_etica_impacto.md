...
- **Avaliação honesta de generalização (OOD).** O **Experimento 3** compara o teste *in-distribution* (Kaggle) com fotos de um **baralho real fotografado pelo próprio autor**, quantificando o **gap de domínio**. Reportar esse gap é a defesa contra a "falsa sensação de robustez".
